...
export function avatar(id,size=''){const p=PEOPLE[id];return `<span class="avatar ${p?.color||'teal'} ${size}" title="${esc(p?.name||id)}">${id==='EG'?icon('robot',16):esc(p?(p.initials||id):id.split(' ').map(s=>s[0]).slice(0,2).join(''))}</span>`;}
