...
import {store,save,reset,event,tick,move,sla,setReviewMode,setDocumentValue,processDocument,decideField,approveDocument} from './store.js';
...
function action(name){const t=currentTask();if(name.startsWith('transition:')){if(t)transition(t,name.split(':')[1]);return;}if(name.startsWith('shell:')){shellPreview(name.slice(6));return;}
 ...
 case 'approve-document':try{approveDocument(t);refresh();toast('Document approved. Requirement satisfied.');}catch(error){toast(error.message);}break;
 ...
 case 'reset-demo':actionModal('Reset the demo?','Restore the original September 9 scenario.',`<p>This clears locally saved comments, status changes, new tasks, and decisions in this prototype.</p>`,{submitLabel:'Reset demo',onSubmit:()=>{reset();view.mode='board';view.sort='rank';detail.key=null;document.querySelector('#task-dialog').close();switchBoard('en-docs');toast('Demo restored. Start with ENR-184.');}});break;
...
document.addEventListener('input',e=>{if(e.target.id==='board-search'){view.q=e.target.value;renderBoard();document.querySelector('.clear-filters').classList.toggle('invisible',!view.q&&view.quick==='all'&&view.owner==='all'&&view.priority==='all'&&view.category==='all');}if(e.target.name==='comment')detail.commentDrafts[detail.key]=e.target.value;if(e.target.id==='request-guidance'){requestState(currentTask()).draft=e.target.value;save();}});
...
document.addEventListener('submit',e=>{
 ...
 if(e.target.id==='action-form'){if(!actionSubmit)return;try{actionSubmit(form);actionDialog.close();actionSubmit=null;}catch(error){document.querySelector('#action-error').textContent=error.message;}return;}
