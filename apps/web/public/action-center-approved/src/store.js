...
import {seedTasks,DEMO_NOW,stageFor,transitionsFor,PEOPLE} from './data.js';
const KEY='audentra.action-center.v1';
export function freshState(){return {revision:2,tasks:seedTasks(),clock:DEMO_NOW};}
function upgrade(s){if(s.revision===2)return s;const seeds=seedTasks();for(const t of s.tasks){const seed=seeds.find(x=>x.key===t.key);if(!seed)continue;for(const e of seed.activity.filter(e=>e.kind==='communication'))if(!t.activity.some(x=>x.text===e.text))t.activity.push(e);if(t.key==='ENR-216'&&t.status==='processing'&&!t.review&&Date.parse(t.updated)<=Date.parse(DEMO_NOW)){t.processingMode='manual';t.owner='AR';t.delegation='Direct assignment';t.labels=['Manual processing'];}}s.revision=2;return s;}
function read(){try {const s=JSON.parse(localStorage.getItem(KEY));if(s?.tasks?.length&&s.clock)return upgrade(s);}catch{} return freshState();}
export const store=typeof localStorage==='undefined'?freshState():read();
export function save(){try{localStorage.setItem(KEY,JSON.stringify(store));}catch{}}
...
export function sla(task){
 if(!task.due)return {label:'SLA met',short:'Completed',tone:'green',remaining:0,percent:100};
 const remaining=(Date.parse(task.due)-Date.parse(store.clock))/3600000;
 const stage=stageFor(task),elapsed=(Date.parse(store.clock)-Date.parse(task.entered))/3600000;
...
export function decideField(task,id,action,note='') {
 ...
 if(!summary.pending.length&&summary.corrections.length){
