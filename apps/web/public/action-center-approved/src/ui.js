...
export function avatar(id,size=''){const p=PEOPLE[id];return `<span class="avatar ${p?.color||'teal'} ${size}" title="${esc(p?.name||id)}">${id==='EG'?icon('robot',16):esc((p?.name||id).split(' ').map(s=>s[0]).slice(0,2).join(''))}</span>`;}
...
let timer;
export function toast(text){const el=document.querySelector('#toast');el.innerHTML=`${icon('info',18)}<span>${esc(text)}</span>`;if(el.showPopover&&!el.matches(':popover-open'))el.showPopover();el.classList.add('visible');clearTimeout(timer);timer=setTimeout(()=>{el.classList.remove('visible');el.hidePopover?.();},4500);}
